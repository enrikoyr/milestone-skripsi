...
// .wrangler/tmp/bundle-OYaWvD/checked-fetch.js
...
// ../.wrangler/tmp/pages-awLieo/functionsWorker-0.0051756029861583785.mjs
...
async function onRequestPatch(context) {
  ...
  try {
    ...
    const timestamp = isCompleted ? (/* @__PURE__ */ new Date()).toISOString() : null;
    await db.prepare(`UPDATE students SET ${column} = ? WHERE id = ?`).bind(timestamp, id).run();
...
async function onRequestPost(context) {
  ...
  try {
    ...
    await db.prepare("INSERT INTO students (id, name, pin, proposalDate) VALUES (?, ?, ?, ?)").bind(id, name, pin, proposalDate).run();
    const newStudent = { id, name, proposalDate, createdAt: (/* @__PURE__ */ new Date()).toISOString() };
...
var routes = [
  {
    routePath: "/api/students/:id",
    mountPath: "/api/students",
    method: "DELETE",
    middlewares: [],
...
// .wrangler/tmp/bundle-OYaWvD/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-OYaWvD/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.0051756029861583785.js.map
